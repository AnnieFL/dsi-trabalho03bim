...
class SpeedrunsController {
    ...
    async cria(req, res) {
        const error = await Util.validate([
            { valor: req.body.runner, nome: "runner" },
            { valor: req.body.tempo, nome: "tempo" },
            { valor: req.body.jogoId, nome: "jogoId" },
            { valor: req.body.categoria, nome: "categoria" }
        ], "AND")
    ...
    async deleta(req, res) {
        ...
        const speedrun = await SpeedrunsModel.destroy({
            where: { id }
        })
